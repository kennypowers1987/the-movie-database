    ...
    http
      ...
      .then(response => {
        ...
        var addToDb = function(time, callback) {
          ...
          db.query("INSERT INTO movies (id, adult, backdrop_path, genre_ids,  original_language, original_title, overview, popularity, poster_path, release_date, title, video, vote_average, vote_count ) ON DUPLICATE KEY UPDATE values ?", [flattenedData], function (err, result) {
            if (err) {
              throw err;
            }
            else {
